...
export NDK="/Users/zhao/Library/Android/ndk/r12b"
if [ ! -d ${NDK} ]
then
...
export TARGET32="android-22"
export TARGET64="android-22"
...
export BASE_CFLAGS='-O2 -g -pedantic -fomit-frame-pointer -Wa,--noexecstack -ffunction-sections -funwind-tables -no-canonical-prefixes -fno-strict-aliasing -fPIC'
...
./configure --prefix=/usr --enable-cxx --disable-assembly --build=x86_64-pc-linux-gnu --host=aarch64-linux-android MPN_PATH="arm64 generic"
sed -i.bak '/HAVE_LOCALECONV 1/d' ./config.h
make -j8 V=1 2>&1 | tee arm64-v8a.log
make install DESTDIR=$PWD/arm64-v8a
cd arm64-v8a && mv usr/lib/libgmp.so usr/lib/libgmpxx.so usr/include/gmp.h usr/include/gmpxx.h usr/lib/libgmp.a usr/lib/libgmpxx.a . && rm -rf usr && cd ..
make distclean
...
./configure --prefix=/usr --enable-cxx --disable-assembly --build=x86_64-pc-linux-gnu --host=arm-linux-androideabi MPN_PATH="arm/v6t2 arm/v6 arm/v5 arm generic"
sed -i.bak '/HAVE_LOCALECONV 1/d' ./config.h
make -j8 V=1 2>&1 | tee armeabi-v7a-neon.log
make install DESTDIR=$PWD/armeabi-v7a-neon
cd armeabi-v7a-neon && mv usr/lib/libgmp.so usr/lib/libgmpxx.so usr/include/gmp.h usr/include/gmpxx.h usr/lib/libgmp.a usr/lib/libgmpxx.a . && rm -rf usr && cd ..
make distclean
...
cd armeabi-v7a && mv usr/lib/libgmp.so usr/lib/libgmpxx.so usr/include/gmp.h usr/include/gmpxx.h usr/lib/libgmp.a usr/lib/libgmpxx.a . && rm -rf usr && cd ..
make distclean

# armeabi
export CFLAGS="${BASE_CFLAGS} -fstack-protector -finline-limit=64 -march=armv5te -mtune=xscale -msoft-float -mthumb"
./configure --prefix=/usr --enable-cxx --disable-assembly --build=x86_64-pc-linux-gnu --host=arm-linux-androideabi MPN_PATH="arm/v5 arm generic"
sed -i.bak '/HAVE_LOCALECONV 1/d' ./config.h
make -j8 V=1 2>&1 | tee armeabi.log
make install DESTDIR=$PWD/armeabi
cd armeabi && mv usr/lib/libgmp.so usr/lib/libgmpxx.so usr/include/gmp.h usr/include/gmpxx.h usr/lib/libgmp.a usr/lib/libgmpxx.a . && rm -rf usr && cd ..
make distclean
